...
germany_loc = find(ismember(cdc_tbl.Country,'Germany'));
other_locs = 1:length(cdc_tbl.Country);
other_locs([niger_loc,italy_loc,monaco_loc,china_loc,germany_loc])=[];
...
germany_coords = 2.4*[no_sus_tbl.ScalingFactor(germany_loc),cdc_tbl.ScalingFactor(germany_loc)];

fig = figure('units','normalized','outerposition',[0 0 1 1]);
plot(2.4*no_sus_tbl.ScalingFactor(other_locs),2.4*cdc_tbl.ScalingFactor(other_locs),'rx','MarkerSize',10);
hold on
plot([niger_coords(1),italy_coords(1),monaco_coords(1),china_coords(1),germany_coords(1)],...
    [niger_coords(2),italy_coords(2),monaco_coords(2),china_coords(2),germany_coords(2)],'k.','MarkerSize',16);
xlim([0 8]);
ylim([0 8]);
...
text(china_coords(1)+0.05,china_coords(2)+0.1,'China','fontsize',14);
text(germany_coords(1)-0.8,germany_coords(2)+0.1,'Germany','fontsize',14);
